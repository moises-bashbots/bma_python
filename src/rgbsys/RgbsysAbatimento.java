...
public class RgbsysAbatimento extends RgbsysSeleniumWebFactView {

    public RgbsysAbatimento(String userName, String password)
    {
        super(userName, password);
    }

    public RgbsysAbatimento()
    {
        super();
    }
...
//                this.driver.get("https://gercloud2.rgbsys.com.br/BMA_HOMOLOGA/operacoes/forms/BaixaSolicitacaoInternet.aspx?SessionId=");
			    ...
			    for (String stringRow : listRows)
			    {
			        ...
			        for (Element row : listTableRows)
			        {
			            ...
			            for (Element rowDescription : listRowDescriptions)
			            {
			                ...
			                // Parsing Abatimento
			                if (mapTableCols.get(rowElementCounter).equals("Abatimento"))
			                {
			                    abatimento = Utils.extractValueFromBrazilianNumber(rowDescription.text());
			                }
			                
			                // Parsing Valor
			                if (mapTableCols.get(rowElementCounter).equals("Valor"))
			                {
			                    valor = Utils.extractValueFromBrazilianNumber(rowDescription.text());
			                }
			            ...
			            TituloInstrucao tituloInstrucao = new TituloInstrucao(empresa, cedente, idTitulo,valor);
			            System.out.println("ValorTitulo: "+tituloInstrucao.getValor());
			            System.out.println("ValorAbatimento: "+abatimento);
			            boolean forbidden=tituloInstrucao.checkProdutoProibido(connMSS); 
			            tituloInstrucao.checkTitulo(connMSS);
			           
			            boolean abatimentoMaiorQueMetade=false;
			            if(abatimento > tituloInstrucao.getValor()/2.0)
			            {
			            	abatimentoMaiorQueMetade=true;
			            	forbidden=true;
			            }
			            System.out.println("AbatimentoMaior: "+abatimentoMaiorQueMetade);
		                ...
		                if(!forbidden && !tituloInstrucao.isAbatido())
		                {
	//                            Thread.sleep(3000);
			                Utils.waitv(3);
	...
	//		                Utils.waitv(10);
		                ...
		                {		                	
		                	if(abatimentoMaiorQueMetade)
		                	{
								System.out.println("Rejeitando ABATIMENTO pois o valor do abatimento é maior do que metade do valor do título");
								...
								this.driver.findElement(By.id("ctl00_contentManager_txtMotivo_I")).sendKeys("Valor do abatimento muito alto!");							
								Utils.waitv(2);
								this.driver.findElement(By.id("ctl00_contentManager_btnMotivo_CD")).click();
								Utils.waitv(4);
		                		
		                	}
		                	else if(tituloInstrucao.isAbatido())
		                	{
								System.out.println("Rejeitando ABATIMENTO pois este título já foi abatido anteriormente");
							    clickId = row.select("input").first().id() + "_D";
							    assert(clickId.contains("ctl00_contentManager_gridLiberacao_DXSelBtn")) : "Id inconsistente";
							    System.out.println("Clicando no elemento" +  clickId);
							    this.driver.findElement(By.id(clickId)).click();
								...
								this.driver.switchTo().frame("ctl00_contentManager_popup_TODOS_CIF-1");
								System.out.println("Rejeitando ABATIMENTO pois este título já foi abatido anteriormente");
								this.driver.findElement(By.id("ctl00_contentManager_txtMotivo_I")).sendKeys("Rejeitando ABATIMENTO pois este título já foi abatido anteriormente!");							
								Utils.waitv(2);
								this.driver.findElement(By.id("ctl00_contentManager_btnMotivo_CD")).click();
								Utils.waitv(4);
...
//		}
    ...
    public static void main(String[] args) {
        ...
        RgbsysAbatimento rgbSysSelenium = new RgbsysAbatimento("MOISES", "moises");
        try
        { 
