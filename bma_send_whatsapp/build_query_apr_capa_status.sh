#!/bin/bash
# Build script for query_apr_capa_status.py executable
# This script builds a standalone binary using PyInstaller and deploys it

set -e  # Exit on error

echo "=========================================="
echo "Query APR CAPA Status - Build Script"
echo "=========================================="
echo ""
...
SPEC_FILE="$SCRIPT_DIR/query_apr_capa_status.spec"
BINARY_NAME="query_apr_capa_status"
...
if [ -f "$SCRIPT_DIR/whatsapp_config.json" ]; then
    cp "$SCRIPT_DIR/whatsapp_config.json" "$DEPLOY_DIR/"
    echo "  ✓ Deployed whatsapp_config.json"
else
    echo -e "${YELLOW}  ⚠ whatsapp_config.json not found - you'll need to create it${NC}"
fi
...
echo "  ./$BINARY_NAME"
echo ""
echo "The program will automatically query records for the current date."
echo ""
